...
Item {
    ...
    readonly property color line: dark ? Qt.rgba(1, 1, 1, 0.1) : "#e5e5ea"
    ...
    readonly property var xSmallItems: {
        ...
        for (let i = 0; i < filteredItems.length; i++) {
            if (filteredItems[i].image)
                pictured.push(filteredItems[i]);
        }
        return pictured.length > 0 ? pictured : filteredItems;
    }
    readonly property var xSmallCurrent: xSmallItems.length > 0 ? xSmallItems[Math.min(xSmallIndex, xSmallItems.length - 1)] : ({})
    ...
    function requestFetch(force) {
        ...
        if (!force && NewsService.feedFresh(sources, categories)) {
            loadCachedNews();
            return;
        }

        if (!force && lastFetchMs > 0 && Date.now() - lastFetchMs < refreshIntervalMs)
            return;
    ...
    function loadCachedNews() {
        const cached = NewsService.cachedFeed(sources, categories);
        if (!cached) {
            items = [];
            lastFetchMs = 0;
            return false;
        }
        return applyNews(JSON.stringify(cached.payload), Number(cached.savedAt || 0));
    }
    ...
    function moveXSmall(step) {
        if (xSmallItems.length < 2 || step === 0)
            return;
    ...
    // Read-only lock interaction. WidgetPreviewFrame supplies only a finite,
    // bounded numeric delta; this function can move the existing viewport (or
    // the compact card index) and cannot invoke refresh/open/AI processes.
    function lockScrollBy(delta) {
        if (typeof delta !== "number" || !Number.isFinite(delta))
            return false;

        let amount = Math.max(-180, Math.min(180, delta));
        if (Math.abs(amount) < 0.01)
            return false;

        if (xSmall) {
            if (xSmallItems.length < 2)
                return false;

            if (xSmallWheelDelta !== 0 && (amount > 0) !== (xSmallWheelDelta > 0))
                xSmallWheelDelta = 0;

            xSmallWheelDelta += amount;
            if (Math.abs(xSmallWheelDelta) >= 48) {
                moveXSmall(xSmallWheelDelta > 0 ? 1 : -1);
                xSmallWheelDelta = 0;
            }
            return true;
        }
        nvEndTimer.stop();
        nvGlide.stop();
        listViewport.cancelFlick();
        let minY = listViewport.originY;
        let maxY = Math.max(minY, minY + listViewport.contentHeight - listViewport.height);
        let nextY = Math.max(minY, Math.min(maxY, listViewport.contentY + amount));
        if (Math.abs(nextY - listViewport.contentY) < 0.01)
            return false;
        listViewport.contentY = nextY;
        return true;
    }
    ...
    function scheduleRowHeightRecalc() {
        if (typeof rowMeasureTimer !== "undefined")
            rowMeasureTimer.restart();
    }
    ...
    function filterItems(src) {
        ...
        for (let i = 0; i < src.length; i++) {
            let it = src[i];
            if (NewsService.has(sources, it.sourceId) && NewsService.has(categories, it.categoryId))
                out.push(it);
        }
        return out;
    }
    ...
    function pruneSummaries() {
        ...
        for (let i = 0; i < filteredItems.length; i++) {
            let url = filteredItems[i].url;
            if (summaryByUrl[url])
                summaries[url] = summaryByUrl[url];

            if (loadingUrls[url])
                loadingMap[url] = true;

            if (errorByUrl[url])
                errors[url] = errorByUrl[url];
        }
    ...
    function countPageSelected() {
        let n = 0;
        for (let i = 0; i < pageItems.length; i++) {
            if (pageItems[i].url && selectedUrls[pageItems[i].url])
                n++;
        }
        return n;
    }
    ...
    function togglePageSelection() {
        ...
        if (pageAllSelected) {
            for (let i = 0; i < pageItems.length; i++) {
                if (pageItems[i].url)
                    delete next[pageItems[i].url];
            }
        } else {
            for (let j = 0; j < pageItems.length; j++) {
                if (pageItems[j].url)
                    next[pageItems[j].url] = true;
            }
        }
        selectedUrls = next;
    }
    ...
    function selectedItems() {
        let out = [];
        for (let i = 0; i < filteredItems.length; i++) {
            if (selectedUrls[filteredItems[i].url])
                out.push(filteredItems[i]);
        }
        return out;
    }
    ...
    function cleanAiText(value) {
        ...
        if (text[0] === "[" || text[0] === "{" || text.indexOf("https://") >= 0 || text.indexOf("\"id\"") >= 0)
            return "";

        if (text.replace(/\s+/g, "").length < 14)
            return "";

        let badStarts = ["며", "으며", "면서", "했다", "됐다", "있다", "없다", "운용할 것을", "운용하기로", "추진하기로", "전장보다", "전날보다", "전일보다", "관련 "];
        for (let i = 0; i < badStarts.length; i++) {
            if (text.indexOf(badStarts[i]) === 0)
                return "";
        }
        let firstWord = text.split(/\s+/)[0].replace(/[.,;:·…!?"'“”‘’]+$/g, "");
        if (/^[가-힣]{1,2}(은|는|이|가|을|를|의|에|도|만|로|으로)?$/.test(firstWord))
            return "";

        if (/^[가-힣]{1,8}(했다|됐다|였다|밝혔다|전했다|말했다|설명했다|마무리했다|했습니다|됐습니다|였습니다|밝혔습니다|전했습니다)$/.test(firstWord))
            return "";
    ...
    function fetchNews() {
        if (!active || fetchProc.running || sources.length === 0 || categories.length === 0)
            return;
    ...
    function applyNews(text, cachedAt) {
        try {
            ...
            lastFetchMs = savedAt > 0 ? savedAt : Date.now();
            if (savedAt <= 0)
                NewsService.storeFeed(sources, categories, j);
    ...
    function startNextSummary() {
        if (aiProc.running || summaryQueue.length === 0)
            return;
    ...
    function applyBatchSummary(text) {
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++)
                byId[rows[i].id] = cleanAiText(rows[i].summary);
    ...
    Timer {
        ...
        interval: 120
        onTriggered: newsRoot.fetchNews()
    }

    Timer {
        interval: newsRoot.refreshIntervalMs
        repeat: true
        running: newsRoot.active
        onTriggered: newsRoot.requestFetch(false)
    }
    ...
    Timer {
        ...
        interval: 1
        onTriggered: {
            if (typeof listCol !== "undefined") {
                listCol.recomputeUniformRowH();
            }
        }
    }
    ...
    Timer {
        ...
        interval: 900
        onTriggered: newsRoot.xSmallPageVisible = false
    }
    ...
    Process {
        ...
        onExited: (code, status) => {
            if (code !== 0 && newsRoot.filteredItems.length === 0)
                newsRoot.status = "뉴스를 불러오지 못했습니다.";
        ...
        stdout: StdioCollector {
            onStreamFinished: newsRoot.applyNews(text, 0)
        }
    }
    ...
    Process {
        ...
        onExited: (code, status) => {
            Qt.callLater(function () {
                if (!newsRoot.batchHandled && newsRoot.activeSummaryUrls.length > 0)
                    newsRoot.markActiveError("Summary failed.");
        ...
        stdout: StdioCollector {
            onStreamFinished: newsRoot.applyBatchSummary(text)
        }
    }
    ...
    Flickable {
        ...
        WheelHandler {
            ...
            onWheel: ev => {
                nvGlide.stop();
                if (Kinetic.onWheel(listViewport, ev, listViewport._ks, {
                    "gain": 90
                }))
                    nvEndTimer.restart();
            }
        ...
        Column {
            ...
            function recomputeUniformRowH() {
                ...
                for (let i = 0; i < newsRepeater.count; i++) {
                    ...
                    if (row)
                        next = Math.max(next, row.contentHeight);
                }
                uniformRowH = next;
            }
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                            if (newsRoot.selectMode)
                                newsRoot.toggleSelected(modelData.url);
                            else
                                newsRoot.openArticle(modelData.url);
                        }
                    }
                }
            }
    ...
    component HeaderButton: Rectangle {
        ...
        scale: hbMa.pressed ? ThemeService.pressScale : 1
    ...
    component HeaderPill: Rectangle {
        ...
        border.width: 1
        scale: hpMa.pressed ? ThemeService.pressScale : 1
    ...
    component PagerButton: Rectangle {
        ...
        scale: pbMa.pressed ? ThemeService.pressScale : 1
