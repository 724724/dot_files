...
Item {
    ...
    readonly property string fontFamily: {
        ...
        return (!f || f === "SF Pro Display" || f === "Pretendard Variable") ? "Apple SD Gothic Neo" : f;
    }
    ...
    function hasContent() {
        return noteRoot._plain(body.text).replace(/\s/g, "").length > 0;
    }

    // Session-lock scrolling enters through WidgetPreviewFrame as a finite,
    // bounded number. It can only move this Flickable; it cannot focus the
    // editor, persist content, or reach any command/process path.
    function lockScrollBy(delta) {
        ...
        let minY = bodyFlick.originY;
        let maxY = Math.max(minY, minY + bodyFlick.contentHeight - bodyFlick.height);
        let nextY = Math.max(minY, Math.min(maxY, bodyFlick.contentY + amount));
        if (Math.abs(nextY - bodyFlick.contentY) < 0.01)
            return false;
        bodyFlick.contentY = nextY;
        return true;
    }

    function requestClose() {
        if (!noteRoot.hasContent()) {
            WidgetsService.removeAt(frame.index);
            return;
        }
        if (noteRoot.d.collapsed)
            frame.save({
                "collapsed": false
            });
        // expand so the dialog fits
        confirming = true;
    }
    ...
    function exportNote(closeAfter) {
        ...
        frame.winRef.openNoteExport(frame.index, "notes-" + ord + ".txt", body.getText(0, body.length), !!closeAfter);
    }
    ...
    Flickable {
        ...
        WheelHandler {
            ...
            onWheel: ev => {
                bfGlide.stop();
                if (Kinetic.onWheel(bodyFlick, ev, bodyFlick._ks, {
                    "gain": 60
                }))
                    bfEndTimer.restart();
            }
        ...
        TextArea.flickable: TextArea {
            ...
            function bumpSize(delta) {
                if (selectionStart !== selectionEnd) {
                    restyleSelection(function (f) {
                        ...
                        let px = f.pixelSize > 0 ? f.pixelSize : (f.pointSize > 0 ? Math.round(f.pointSize * 96 / 72) : noteRoot.fontSize);
                        f.pixelSize = Math.max(WidgetsService.minFont, Math.min(WidgetsService.maxFont, px + delta));
                    });
                } else {
                    let v = Math.max(WidgetsService.minFont, Math.min(WidgetsService.maxFont, noteRoot.fontSize + delta));
                    frame.save({
                        "fontSize": v
                    });
                }
            ...
            Keys.onPressed: ev => {
                ...
                case Qt.Key_B:
                    ...
                    body.restyleSelection(f => {
                        return f.bold = !f.bold;
                    });
                    ev.accepted = true;
                    break;
                case Qt.Key_I:
                ...
                case Qt.Key_U:
                    ...
                    body.restyleSelection(f => {
                        return f.underline = !f.underline;
                    });
                    ev.accepted = true;
                    break;
                case Qt.Key_S:
                    if (shift) {
                        body.restyleSelection(f => {
                            return f.strikeout = !f.strikeout;
                        });
                    } else {
                        saveTimer.stop();
                        noteRoot.exportNote(false);
                    }
                    ev.accepted = true;
                    break; // "+" usually needs Shift, so accept both = and + (and _ / -).
                case Qt.Key_Plus:
                case Qt.Key_Equal:
                    body.bumpSize(1);
                    ev.accepted = true;
        ...
        Rectangle {
            ...
            Column {
                ...
                DialBtn {
                    ...
                    primary: true
                    onClicked: {
                        noteRoot.confirming = false;
                        noteRoot.exportNote(true);
                    }
                }
    ...
    component DialBtn: Rectangle {
        ...
        height: 30
        radius: 8
        color: primary ? (dbHover.hovered ? Qt.rgba(0.3, 0.52, 0.95, 1) : Qt.rgba(0.3, 0.52, 0.95, 0.85)) : (dbHover.hovered ? Qt.rgba(1, 1, 1, 0.18) : Qt.rgba(1, 1, 1, 0.09))
        scale: dialMa.pressed ? ThemeService.pressScale : 1
