...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("wineserver -p")
    hl.exec_cmd("hyprsunset")
    hl.exec_cmd("systemctl --user start graphical-session.target")
    hl.exec_cmd("sleep 5 && sudo pkill sddm-greeter")

    -- gdrive
    hl.exec_cmd("sleep 5 && rclone mount gdrive: /mnt/gdrive --vfs-cache-mode full --vfs-cache-max-size 50G --vfs-cache-max-age 72h --dir-cache-time 72h --buffer-size 128M --vfs-read-chunk-size 128M --vfs-fast-fingerprint --daemon")
end)
