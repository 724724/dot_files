...
function OsdWindow() {
	...
	if (mode() === "mic") return muted() ? "microphone-disabled-symbolic" : "audio-input-microphone-symbolic"
	if (muted() || percent() === 0) return "audio-volume-muted-symbolic"
	return "audio-volume-high-symbolic"
    })

    const percentText = createComputed(() => {
	if ((mode() === "volume" || mode() === "mic") && muted()) return "Muted"
        return `${percent()}%`
    })

    const progressValue = createComputed(() => {
	if ((mode() === "volume" || mode() === "mic") && muted()) return 0
        return percent() / 100
    })
    ...
    return (
        ...
        >
            <box halign={Gtk.Align.CENTER} hexpand={false}>
                <box
                    class="osd-container osd-mic"
                    visible={createComputed(() => mode() === "mic")}
                    halign={Gtk.Align.CENTER}
                >
                    <image
                        iconName={createComputed(() => muted() ? "microphone-disabled-symbolic" : "audio-input-microphone-symbolic")}
                        class="osd-icon"
                        valign={Gtk.Align.CENTER}
                    />
                    <label
                        label={createComputed(() => muted() ? "Muted" : "On")}
                        class="osd-label"
                        valign={Gtk.Align.CENTER}
                    />
                </box>
